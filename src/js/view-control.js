// View control logic
import { addClass, addEventListener, removeClass } from './utils/helpers.js';

export function initializeViewControl(
	viewListBtn,
	viewBothBtn,
	viewMapBtn,
	map,
	...
	function setViewMode(mode) {
		...
		for (const btn of viewControlButtons) {
			if (btn) {
				// Ensure button exists before trying to modify classList
				removeClass(btn, 'active');
				if (
					(mode === 'list' && btn === viewListBtn) ||
					(mode === 'map' && btn === viewMapBtn) ||
					((mode === 'both' || !mode) && btn === viewBothBtn)
				) {
					// Default to 'both'
					addClass(btn, 'active');
				}
