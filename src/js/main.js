...
import { addEventListener, getElement } from './utils/helpers.js';
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const setViewMode = initializeViewControl(
		viewListBtn,
		viewBothBtn,
		viewMapBtn,
		map,
	...
	try {
		...
		if (establecimientos && establecimientos.length > 0) {
			...
			listElement.innerHTML =
				'<li class="no-results">No se encontraron establecimientos en los datos.</li>';
		}
	} catch (error) {
		console.error('Error al cargar los datos:', error);
		listElement.innerHTML =
			'<li class="no-results">Error al cargar los datos. Por favor, intente más tarde.</li>';
	}
});
