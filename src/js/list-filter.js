...
export function initializeListFilter(
	...
	function getEstablishmentTypes(items) {
		...
		for (const item of items) {
			...
			// Tipos principales
			if (
				nombre.startsWith('DISPENSARIO') ||
				nombre.includes(' DISPENSARIO ')
			) {
	...
	function displayList(items) {
		...
		if (!items || items.length === 0) {
			setInnerHTML(
				listElement,
				'<li class="no-results">No se encontraron establecimientos.</li>',
			);
			return;
		}
		...
		for (const [index, item] of items.entries()) {
			...
			addEventListener(li, 'click', () => {
				...
				const listItems = document.querySelectorAll('#establishment-list li');
		...
		const filteredItems = establecimientos.filter((item) => {
			...
			if (
				...
				switch (selectedType) {
					...
					case 'Consultorio':
						return (
							nombre.startsWith('CONSULTORIO') ||
							nombre.includes(' CONSULTORIO ')
						);
					case 'Centro':
						return (
							nombre.includes('CENTRO DE') || nombre.includes('CENTRO DE SALUD')
						);
					case 'Escuela':
	...
	if (document.getElementById('clear-search')) {
		addEventListener(document.getElementById('clear-search'), 'click', () =>
			clearFilters('search'),
		);
	}
	if (document.getElementById('clear-city')) {
		addEventListener(document.getElementById('clear-city'), 'click', () =>
			clearFilters('city'),
		);
	}
	if (document.getElementById('clear-type')) {
		addEventListener(document.getElementById('clear-type'), 'click', () =>
			clearFilters('type'),
		);
	}
